...
function betMultiplier(bet: BetType): number {
  switch (bet.type) {
    ...
    case "dozen": case "row2:1": return 3;
  }
}
...
// ---------- SVG Wheel ----------
const WheelSVG = ({ rotation, transition }: { rotation: number; transition: string }) => {
  const size = 300, cx = 150, cy = 150;
  const rOuter = 145, rInner = 65, rLabel = 122;
  const arc = (start: number, end: number, rO: number, rI: number) => {
    const s = (a: number) => [cx + rO * Math.cos(a), cy + rO * Math.sin(a)];
    const e = (a: number) => [cx + rI * Math.cos(a), cy + rI * Math.sin(a)];
    const sa = (start - 90) * Math.PI / 180;
    const ea = (end - 90) * Math.PI / 180;
    const [x1, y1] = s(sa), [x2, y2] = s(ea);
    const [x3, y3] = e(ea), [x4, y4] = e(sa);
  ...
  return (
    <svg
      ...
      style={{ transform: `rotate(${rotation}deg)`, transition, transformOrigin: "center" }}
    >
      <defs>
      ...
      <circle cx={cx} cy={cy} r={rOuter + 4} fill="#3f2c10" />
      <circle cx={cx} cy={cy} r={rOuter + 2} fill="#1a1a2e" />
      {WHEEL_ORDER.map((n, i) => {
        const start = i * SLICE;
        const end = start + SLICE;
        const c = getColor(n);
        const fill = c === "red" ? "#dc2626" : c === "green" ? "#16a34a" : "#18181b";
        const mid = (start + end) / 2;
        const ma = (mid - 90) * Math.PI / 180;
        const lx = cx + rLabel * Math.cos(ma);
        const ly = cy + rLabel * Math.sin(ma);
        return (
          <g key={n}>
            <path d={arc(start, end, rOuter, rInner)} fill={fill} stroke="#000" strokeWidth="0.5" />
            <text
              x={lx} y={ly}
      ...
      <circle cx={cx} cy={cy} r={rInner} fill="url(#hub)" stroke="#a16207" strokeWidth="2" />
      <text x={cx} y={cy + 5} fill="#facc15" fontSize="28" textAnchor="middle">✦</text>
    </svg>
  );
};
...
const RouletteGame = ({ balance, setBalance, onGameResult }: Props) => {
  ...
  // Wheel and ball rotations (cumulative)
  const [wheelRot, setWheelRot] = useState(0);
  const [ballRot, setBallRot] = useState(0);
  const [wheelTrans, setWheelTrans] = useState("none");
  const [ballTrans, setBallTrans] = useState("none");
  const [ballRadius, setBallRadius] = useState(135); // px from center, animated
  ...
  const runSpin = useCallback((currentBets: PlacedBet[]) => {
    ...
    // Slot's center angle in the wheel's local frame (0° = top, clockwise).
    const slotAngle = idx * SLICE + SLICE / 2;

    // Wheel spins clockwise (positive). Add full revolutions.
    const wheelTurns = 5;
    const newWheelRot = wheelRot + wheelTurns * 360;

    // Ball spins counter-clockwise (negative). It must end so that
    // (ballAngleWorld) - (newWheelRot) ≡ slotAngle  (mod 360)
    // i.e. ball world angle = newWheelRot + slotAngle (+ extra ccw turns)
    const ballTurns = 7;
    const targetWorld = newWheelRot + slotAngle;
    // ensure ballRot moves backward (negative direction) by enough turns
    const currentBallMod = ((ballRot % 360) + 360) % 360;
    const targetMod = ((targetWorld % 360) + 360) % 360;
    let delta = targetMod - currentBallMod; // could be negative
    // We want net negative rotation (ccw) of about ballTurns full spins
    while (delta > -360) delta -= 360;
    delta -= (ballTurns - 1) * 360;
    const newBallRot = ballRot + delta;

    // Apply transitions
    const ease = "cubic-bezier(0.17, 0.67, 0.21, 1)";
    setWheelTrans(`transform ${SPIN_MS}ms ${ease}`);
    setBallTrans(`transform ${SPIN_MS}ms ${ease}`);
    requestAnimationFrame(() => {
      setWheelRot(newWheelRot);
      setBallRot(newBallRot);
    });

    // Ball drops inward over the spin
    setBallRadius(135);
    setTimeout(() => setBallRadius(115), 50);
    setTimeout(() => setBallRadius(95), SPIN_MS * 0.6);
    setTimeout(() => setBallRadius(82), SPIN_MS * 0.85);

    // Ticking sounds
    tickTimers.current.forEach(t => clearTimeout(t));
    tickTimers.current = [];
    const ticks = [0.2, 0.5, 0.8, 1.1, 1.45, 1.8, 2.15, 2.5, 2.85, 3.15, 3.45, 3.7, 3.9, 4.05, 4.2, 4.35, 4.5, 4.65, 4.8];
    ticks.forEach((t, i) => {
    ...
    setTimeout(() => {
      ...
      onGameResult({
        game: "Roulette", bet: tb, payout: totalPayout,
        won: totalPayout > 0,
  ...
  }, [wheelRot, ballRot, setBalance, onGameResult, playIfOn]);
  ...
  const startAuto = useCallback(() => {
    if (spinning || autoRunning.current || bets.length === 0) return;
  ...
  useEffect(() => {
    ...
    setBets(replay);
    const id = window.setTimeout(() => runSpin(replay), 1000);
  ...
  // Wheel container size in px (must match ball math)
      ...
      <div className="mx-auto max-w-6xl flex flex-col lg:flex-row gap-4">
        ...
        <div className="w-full lg:w-72 shrink-0 space-y-4">
          <div className="rounded-xl border border-white/5 bg-[hsl(222,47%,9%)] p-4 space-y-4 shadow-[0_0_30px_rgba(15,23,42,0.5)]">
            <div className="flex rounded-lg overflow-hidden border border-white/5 bg-[hsl(222,47%,6%)] p-1">
              <button onClick={() => setMode("manual")} className={`flex-1 py-2 text-sm font-semibold rounded-md transition-all ${mode === "manual" ? "bg-[hsl(217,91%,60%)] text-white shadow-[0_0_15px_rgba(56,189,248,0.5)]" : "text-slate-400 hover:text-white"}`}>Manuel</button>
              <button onClick={() => setMode("auto")} className={`flex-1 py-2 text-sm font-semibold rounded-md transition-all ${mode === "auto" ? "bg-[hsl(217,91%,60%)] text-white shadow-[0_0_15px_rgba(56,189,248,0.5)]" : "text-slate-400 hover:text-white"}`}>Auto</button>
            </div>

            <div>
              <p className="text-xs text-slate-400 mb-2">Valeur du Jeton <span className="font-bold text-white">{selectedChip.toLocaleString("fr-FR")}</span></p>
              <div className="flex items-center gap-2">
                {CHIPS.map(chip => (
                  <button key={chip} onClick={() => setSelectedChip(chip)} className={`w-10 h-10 rounded-full text-xs font-bold transition-all ${selectedChip === chip ? "bg-yellow-400 text-black ring-2 ring-yellow-300 scale-110 shadow-[0_0_15px_rgba(250,204,21,0.7)]" : "bg-yellow-700/40 text-yellow-200 hover:bg-yellow-600/60"}`}>{chip >= 1000 ? `${chip / 1000}K` : chip}</button>
                ))}
              </div>
            </div>
            ...
            {mode === "auto" && (
              <div className="space-y-2 border-t border-white/5 pt-3">
                ...
                <div className="flex gap-2">
                  {[10, 25, 50, 100].map(n => (
                    <button key={n} onClick={() => setAutoSpins(n)} className={`flex-1 py-1.5 rounded text-xs font-semibold transition-all ${autoSpins === n ? "bg-[hsl(217,91%,60%)] text-white" : "bg-[hsl(222,47%,6%)] text-slate-300 hover:bg-[hsl(222,47%,12%)]"}`}>{n}</button>
                  ))}
                </div>
                {autoRemaining > 0 && <p className="text-xs text-center text-cyan-400">Restants : {autoRemaining}</p>}
              </div>
            )}

            {mode === "manual" ? (
              <button onClick={spin} disabled={spinning || bets.length === 0} className="w-full py-3 rounded-lg font-bold text-base transition-all bg-[hsl(217,91%,60%)] text-white hover:bg-[hsl(217,91%,65%)] hover:shadow-[0_0_20px_rgba(56,189,248,0.6)] disabled:opacity-40 disabled:cursor-not-allowed">{spinning ? "En cours..." : "Lancer"}</button>
            ) : autoRemaining > 0 ? (
              <button onClick={stopAuto} className="w-full py-3 rounded-lg font-bold text-base bg-red-600 text-white hover:bg-red-500 hover:shadow-[0_0_20px_rgba(239,68,68,0.6)] transition-all">Arrêter</button>
            ) : (
              <button onClick={startAuto} disabled={spinning || bets.length === 0} className="w-full py-3 rounded-lg font-bold text-base bg-[hsl(217,91%,60%)] text-white hover:bg-[hsl(217,91%,65%)] hover:shadow-[0_0_20px_rgba(56,189,248,0.6)] disabled:opacity-40 disabled:cursor-not-allowed transition-all">Démarrer Auto</button>
            )}

            <button onClick={() => setSoundOn(s => !s)} className="w-full py-1.5 rounded text-xs font-semibold bg-[hsl(222,47%,6%)] hover:bg-[hsl(222,47%,12%)] text-slate-300 border border-white/5">Son : {soundOn ? "ON 🔊" : "OFF 🔇"}</button>
          </div>
          ...
          {history.length > 0 && (
            <div className="rounded-xl border border-white/5 bg-[hsl(222,47%,9%)] p-3">
              ...
              <div className="flex flex-wrap gap-1.5">
                {history.map((n, i) => (
                  <span key={i} className={`w-7 h-7 flex items-center justify-center rounded-full text-[11px] font-bold text-white ${getColor(n) === "red" ? "bg-red-600" : getColor(n) === "green" ? "bg-emerald-600" : "bg-zinc-800"} ${i === 0 ? "ring-2 ring-yellow-400 shadow-[0_0_10px_rgba(250,204,21,0.6)]" : ""}`}>{n}</span>
                ))}
              </div>
            </div>
          )}
        ...
        <div className="flex-1 space-y-5">
          <div className="flex justify-center">
            <div className="relative" style={{ width: WHEEL_SIZE, height: WHEEL_SIZE }}>
              ...
              <div className="absolute inset-0">
                <WheelSVG rotation={wheelRot} transition={wheelTrans} />
              </div>
              ...
              >
                <div
                  className="absolute left-1/2 -translate-x-1/2 w-[14px] h-[14px] rounded-full bg-white shadow-[0_0_12px_rgba(255,255,255,0.95)]"
                  style={{
                    top: `${WHEEL_SIZE / 2 - ballRadius - 7}px`,
                    transition: spinning ? `top ${SPIN_MS}ms cubic-bezier(0.4, 0, 0.6, 1)` : "top 0.3s ease",
              ...
              {result !== null && !spinning && (
                <div className="absolute inset-0 flex items-center justify-center z-30 pointer-events-none">
                  <div className={`w-20 h-20 rounded-full flex items-center justify-center text-3xl font-extrabold text-white animate-scale-in ${getColor(result) === "red" ? "bg-red-600 shadow-[0_0_30px_rgba(220,38,38,0.8)]" : getColor(result) === "green" ? "bg-emerald-600 shadow-[0_0_30px_rgba(16,185,129,0.8)]" : "bg-zinc-800 shadow-[0_0_30px_rgba(255,255,255,0.4)]"}`}>{result}</div>
                </div>
              )}
            </div>
          </div>
            ...
            <div className="min-w-[620px]">
              <div className="grid grid-cols-[40px_repeat(12,1fr)_40px] gap-[2px]">
                <div className="row-span-3">
                  <button onClick={() => placeBet({ type: "number", value: 0 })} disabled={spinning} className="relative w-full h-full flex items-center justify-center font-bold text-sm bg-emerald-600 hover:bg-emerald-500 hover:shadow-[0_0_12px_rgba(16,185,129,0.5)] text-white border border-white/5 rounded-l-md transition-all disabled:opacity-70">
                    0
                    {getBetAmount({ type: "number", value: 0 }) > 0 && (
                      <span className="absolute -top-1 -right-1 z-10 min-w-[18px] h-[18px] flex items-center justify-center rounded-full bg-yellow-400 text-[10px] font-bold text-black px-1 shadow-[0_0_8px_rgba(250,204,21,0.7)]">{getBetAmount({ type: "number", value: 0 })}</span>
                    )}
                  </button>
                </div>
              ...
              <div className="grid grid-cols-6 gap-[2px] mt-[2px] ml-[42px] mr-[42px]">
                ...
                <button onClick={() => placeBet({ type: "red" })} disabled={spinning} className="relative h-9 bg-red-600 hover:bg-red-500 hover:shadow-[0_0_12px_rgba(220,38,38,0.5)] border border-white/5 transition-all disabled:opacity-70">
                  {getBetAmount({ type: "red" }) > 0 && (<span className="absolute -top-1 -right-1 z-10 min-w-[18px] h-[18px] flex items-center justify-center rounded-full bg-yellow-400 text-[10px] font-bold text-black px-1 shadow-[0_0_8px_rgba(250,204,21,0.7)]">{getBetAmount({ type: "red" })}</span>)}
                </button>
                <button onClick={() => placeBet({ type: "black" })} disabled={spinning} className="relative h-9 bg-zinc-900 hover:bg-zinc-800 hover:shadow-[0_0_12px_rgba(255,255,255,0.2)] border border-white/5 transition-all disabled:opacity-70">
                  {getBetAmount({ type: "black" }) > 0 && (<span className="absolute -top-1 -right-1 z-10 min-w-[18px] h-[18px] flex items-center justify-center rounded-full bg-yellow-400 text-[10px] font-bold text-black px-1 shadow-[0_0_8px_rgba(250,204,21,0.7)]">{getBetAmount({ type: "black" })}</span>)}
                </button>
          ...
          <div className="flex items-center justify-between px-1">
            <button onClick={undoLastBet} disabled={spinning || bets.length === 0} className="flex items-center gap-1.5 text-sm text-slate-400 hover:text-white transition-colors disabled:opacity-40">↩ Annuler</button>
            <button onClick={clearBets} disabled={spinning || bets.length === 0} className="flex items-center gap-1.5 text-sm text-slate-400 hover:text-white transition-colors disabled:opacity-40">Effacer ↻</button>
          </div>
        </div>
      </div>
    </main>
